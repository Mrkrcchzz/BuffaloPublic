(async function() {
  // 1. Google Sheet CSV Export Link
  const SHEET_CSV_URL = 'https://docs.google.com/spreadsheets/d/1TfwCSGBMi6-6NZfP5vWAKbkrWlDjEsSeZTEr-4_5yRE/export?format=csv';

  try {
    // 2. Fetch CSV Data from Google Sheet
    const response = await fetch(SHEET_CSV_URL);
    if (!response.ok) throw new Error('Could not fetch sheet data. Check Google Sheet sharing permissions.');
    const text = await response.text();

    // 3. Parse IDs from Column A
    ...
    // 4. Locate Infinite Campus Student Elements
